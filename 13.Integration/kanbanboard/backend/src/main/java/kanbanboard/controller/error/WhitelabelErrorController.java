...
public class WhitelabelErrorController implements ErrorController {
  ...
  public ResponseEntity<JsonResult<String>> _404() {
    return ResponseEntity.status(404).body(JsonResult.fail("unknown url"));
  }


  @RequestMapping("/500")
  public ResponseEntity<JsonResult<String>> _500(@RequestAttribute String errors) {
    return ResponseEntity.internalServerError().body(JsonResult.fail(errors));
  }

  @GetMapping
  public ResponseEntity<JsonResult<String>> handleError(HttpServletRequest request) {
    return ResponseEntity
        .status(Optional.ofNullable(request.getAttribute(RequestDispatcher.ERROR_STATUS_CODE))
            .map(statusCode -> Integer.valueOf(statusCode.toString())).orElse(520))
